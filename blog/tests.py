...
class BlogPostTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.post1 = Post.objects.create(
            ...
            status=Post.STATUS_CHOICES[0][0], # or 'pub' # published
            author=cls.user,
        )
    ...
    def test_status_404_if_post_id_does_not_exist(self):
        response = self.client.get(reverse('post_detail', args=[1000]))
        self.assertEqual(response.status_code, 404)

    def test_draft_post_not_show_in_post_list(self):  # TDD --> Test Driven Development
        response = self.client.get(reverse('posts_list'))
        self.assertContains(response, self.post1.title)
